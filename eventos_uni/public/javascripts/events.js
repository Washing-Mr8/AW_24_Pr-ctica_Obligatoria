...
$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        <div class="col-12 col-sm-12 col-md-6 col-lg-4">
          <div class="card card-hover h-100 ${newEvent.Fecha < new Date() ? 'bg-warning text-black' : 'card-content'}" id="event-${newEvent.ID}">
            <div class="card-body">
              <h4 class="card-title mb-1">${newEvent.Titulo} ${newEvent.Fecha < new Date() ? '(TERMINADO)' : ''}</h4>
              <p class="card-text"><strong>Descripción:</strong> ${newEvent.Descripcion}</p>
              <p class="card-text"><strong>Fecha:</strong> ${new Date(newEvent.Fecha).toISOString().split('T')[0]} <strong>Hora:</strong> ${newEvent.Hora}</p>
              <p class="card-text"><strong>Duración:</strong> ${newEvent.Duracion}</p>
              <p class="card-text"><strong>Ubicación:</strong> ${newEvent.Facultad}: ${newEvent.Ubicacion}</p>
              <p class="card-text"><strong>Capacidad:</strong> ${newEvent.Capacidad_Actual} / ${newEvent.Capacidad_Maxima} personas</p>
              <p class="card-text"><strong>Tipo de Evento:</strong> ${newEvent.Tipo}</p>
      ...
      `
          ...
          $('.events-container .row').prepend(eventHtml);
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (filterForm) {
      // Si existe, se añade el eventListener
          ...
          success: function (response) {
            if (response.success) {
              ...
              events.forEach(Event => {
                ...
                let eventHtml = `
              <div class="col-12 col-sm-12 col-md-6 col-lg-4">
            <div class="card card-hover h-100 ${isPastEvent ? 'bg-warning text-black' : 'card-content'}" id="event-${Event.ID}">
              <div class="card-body">
                <h4 class="card-title mb-1">${Event.Titulo} ${isPastEvent ? '(TERMINADO)' : ''}</h4>
                <p class="card-text"><strong>Descripción:</strong> ${Event.Descripcion}</p>
                <p class="card-text"><strong>Fecha:</strong> ${new Date(Event.Fecha).toISOString().split('T')[0]} <strong>Hora:</strong> ${Event.Hora}</p>
                <p class="card-text"><strong>Duración:</strong> ${Event.Duracion}</p>
                <p class="card-text"><strong>Ubicación:</strong> ${Event.facultad}:${Event.Ubicacion}</p>
                <p class="card-text"><strong>Capacidad:</strong> ${Event.Capacidad_Actual} / ${Event.Capacidad_Maxima} personas</p>
                <p class="card-text"><strong>Tipo de Evento:</strong> ${Event.tipo}</p>`;
                ...
                if (user[0].Rol === "organizador") {
                  ...
                  if (estadoInscripcion === "inscrito" && isPastEvent) {
                    eventHtml += `
                      <button class="btn btn-danger btn-sm" data-bs-target="#commentEventModal${Event.ID}" id="leaveEvent${Event.ID}" data-bs-toggle="modal">Desinscribirse</button></p>`;
                  }
                }
